...
- This package is focused on native iOS Liquid Glass components. When a feature is requested as native iOS, prefer SwiftUI/UIKit implementations bridged into Flutter over Dart-only visual simulations.
- Keep native iOS component code split for readability:
    - SwiftUI rendering/config in `*View.swift` files.
    - Platform-view bridge/factory logic in `*ViewFactory.swift` files.
- In Flutter widget classes, enforce method ordering:
    - Place non-widget functions (logic, channel handlers, parsing helpers, utility methods) above `build()`.
    - Place widget-construction helper methods (for example `_buildXyz`) below `build()`.
