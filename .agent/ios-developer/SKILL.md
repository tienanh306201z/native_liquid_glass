...
- For this repository, any Liquid Glass UI component requested as "native iOS" must be implemented through iOS native code (SwiftUI/UIKit) and exposed to Flutter via platform views or channels.
- Do not substitute core native iOS Liquid Glass components with Dart-only approximations unless explicitly requested as fallback behavior.
- For readability and maintainability, split each native component into separate Swift files:
    - `<Component>View.swift` for SwiftUI rendering/config types.
    - `<Component>ViewFactory.swift` for Flutter bridge/factory/platform-view plumbing.
