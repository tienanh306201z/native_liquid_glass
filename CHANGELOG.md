...
### `LiquidGlassNavigationBar` — new `brightness` option to pin the bar to the app theme

- The bar resolves its dynamic colors (title, bar-button glyphs, glass background) against the **device** appearance, like any UIKit bar. Apps that drive their own `ThemeMode` could end up with a white title on a light page — the "title is always white in light mode" report in [#17](https://github.com/tienanh306201z/native_liquid_glass/pull/17) ([#20](https://github.com/tienanh306201z/native_liquid_glass/issues/20)). `LiquidGlassNavigationBar.brightness` now lets you pass `Theme.of(context).brightness`; the value is sent in `creationParams` and pushed live over `setBrightness` when it changes, without recreating the platform view. **Default is unchanged**: leave it null and the bar keeps following the device. Verified on the iOS 27 simulator with the device appearance forced to dark, plus four XCTests.
